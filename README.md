...
3. 设定定时时间（默认 10:00:00），到点自动开抢，**无需刷新页面**
4. 面板显示倒计时，按钮从"暂时售罄"恢复为"特惠订阅"后即刻可点
5. 抢到后自动拉起支付弹窗，手动完成付款

> 关键：提前打开页面挂着，脚本持续修正按钮状态。十点补货按钮变亮时页面不刷新也能直接抢。
...
- **反售罄补丁**: 拦截页面 API 响应（XHR + Vue 数据 + DOM 三层），修正 `isSoldOut`/`isServerBusy` 状态，按钮从"暂时售罄"恢复为"特惠订阅"
- **免刷新抢购**: 页面打开挂着即可，补货后按钮自动恢复可点，无需手动刷新
- **定时抢购**: 到点自动开抢，面板实时显示倒计时
